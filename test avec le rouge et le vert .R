...
revenu_text2 <- paste("Département avec le plus bas revenu moyen :", min_revenu_dep$nom, "(", round(min_revenu_dep$Revenu, 0), "€)")
...
chomage_text2 <- paste("Département avec le taux de chômage le plus bas :", min_chomage_dep$nom, "(", round(min_chomage_dep$Chomage, 1), "%)")
...
transport_text2 <- paste("Département avec le plus bas indice de transport :", min_transport_dep$nom, "(", round(min_transport_dep$Transport, 0), ")")
...
server <- function(input, output, session) {
  ...
  output$map_chomage <- renderLeaflet({
    leaflet(departements_sf) %>%
      ...
      addPolygons(data = highest_chomage_dep, color = "red", weight = 3, fillOpacity = 0, 
                  popup = ~paste("<strong>", nom, "</strong><br/>Taux de chômage :", round(Chomage, 1), "%")) %>%
      addPolygons(data = min_chomage_dep, color = "green", weight = 3, fillOpacity = 0, 
                  popup = ~paste("<strong>", nom, "</strong><br/>Taux de chômage :", round(Chomage, 1), "%")) %>%
  ...
  output$map_revenu <- renderLeaflet({
    leaflet(departements_sf) %>%
      ...
      addPolygons(data = max_revenu_dep, color = "green", weight = 3, fillOpacity = 0, 
                  popup = ~paste("<strong>", nom, "</strong><br/>Revenu moyen (max) :", round(Revenu, 0), " €")) %>%
      addPolygons(data = min_revenu_dep, color = "red", weight = 3, fillOpacity = 0, 
                  popup = ~paste("<strong>", nom, "</strong><br/>Revenu moyen (min) :", round(Revenu, 0), " €")) %>%
  ...
  output$map_transport <- renderLeaflet({
    leaflet(departements_sf) %>%
      ...
      addPolygons(data = max_transport_dep, color = "green", weight = 3, fillOpacity = 0, 
                  popup = ~paste("<strong>", nom, "</strong><br/>Indice de Transport :", round(Transport, 0))) %>%
  ...
  output$map_construction <- renderLeaflet({
    leaflet(departements_sf) %>%
      ...
      addPolygons(data = max_construction_dep, color = "green", weight = 3, fillOpacity = 0, 
                  popup = ~paste("<strong>", nom, "</strong><br/>Taux de Construction :", round(construction, 2))) %>%
